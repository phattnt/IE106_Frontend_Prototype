...
module.exports = async function scanContrast(page, label = 'page') {
  return page.evaluate((scanLabel) => {
    ...
    function parseColor(value) {
      ...
      const channels = parts.slice(0, 3).map((part) => {
        if (part.endsWith('%')) {
          return clamp((Number.parseFloat(part) / 100) * 255)
        }
    ...
    function isVisibleElement(element) {
      if (!element || element.closest('.material-symbols-outlined')) {
        return false
      }
